...
function Searchpanel() {
  ...
  return (
    <div className="md: m-auto w-[100%] max-w-7xl px-5 md:px-0">
      ...
      {session?.user?.email === 'deepak.e11296@cumail.in' ||
      session?.user?.email === 'pl.aitaiml@cumail.in' ||
      session?.user?.email === 'Deepak.e11296@cumail.in' ||
      session?.user?.email === 'anujsinghsisodiya5341@gmail.com' ? (
        modal === true ? (
          <Absent />
        ) : (
          <>
        ...
        <p className="mt-20 text-center text-[2.5rem] font-semibold italic text-gray-400  ">
          Access not allowed ⚠️
        </p>
      )}
    </div>
  )
